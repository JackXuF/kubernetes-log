...
func assertRequiredOptions() {
	if options.serverAddr == "" {
		exit(exitStat.usageError, "fatal: server address must be defined")
	}
}
...
func init() {
	flag.StringVar(&options.serverAddr, "server", options.serverAddr, "the IP address of master node")
...
func main() {
	...
	var config Config
	config.Network.Servers = append(config.Network.Servers, options.serverAddr+":3333")
	/*
		additional_config, err := LoadConfig(options.configArg)
		if err == nil {
			err = MergeConfig(&config, additional_config)
		}
		if err != nil {
			fault("Could not load config file %s: %s", options.configArg, err)
		}
	*/
	...
	filelist_chan := make(chan Pick)

	//request etcd to get log file paths
	/*
		if len(config.Files) != 0 {
			log.Println("paths will be ignored, since paths will be requested from etcd now\n")
		}
	*/
	go GetFileList(filelist_chan, options.serverAddr)
	go clearLog(options.serverAddr)
	/*
		if len(config.Files) == 0 {
	...
	*/
	...
	go prospector.MyProspect(restart, event_chan, filelist_chan)
