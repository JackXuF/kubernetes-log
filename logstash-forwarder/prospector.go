...
func (p *Prospector) MyProspect(resume *ProspectorResume, output chan *FileEvent, filelist chan Pick) {
	p.prospectorinfo = make(map[string]ProspectorInfo)

	pick := <-filelist

	p.FileConfig.Paths = pick.Filelist
...
func (p *Prospector) myScan(file string, output chan *FileEvent, resume *ProspectorResume) {
